...
const BG_LAYERS = [
  { color: 0x16163a, speed: 0.0, rects: [] }, // cielo estático
  {
    color: 0x1e1e52,
    speed: 0.15,
    rects: generateBgRects(0.15, 8, 2, 80, 200, 60, 120),
  },
  {
    color: 0x28285a,
    speed: 0.35,
    rects: generateBgRects(0.35, 12, 80, 160, 30, 80),
  },
];

function generateBgRects(
  speed,
  count,
  yMin,
  yMax,
...
const game = {
  ...
  async init() {
    ...
    hiText = new PIXI.Text("Mejor: 0", {
      ...style,
      fontSize: 14,
      fill: 0x4ecca3,
    });
    msgText = new PIXI.Text("", {
      ...style,
      fontSize: 28,
      fill: 0xe94560,
      fontWeight: "bold",
  ...
  async _loadCharacter() {
    ...
    for (const [animName, def] of Object.entries(ANIMATIONS)) {
      manager.createAnimation("hero", animName, def.frames, {
        speed: def.speed ?? 10,
      });
    }
