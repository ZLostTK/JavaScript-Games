const game = {
	...
	update(dt) {
		if (this.state === 'playing') {
			...
			if (currentPlayer && currentPlayer.isAI) {
				...
				this._aiTimer += dt;
				if (this._aiTimer > 1.5) { // 1.5s AI delay
					this._aiTimer = 0;
					this._playAITurn();
				}
			}
	...
	_buildMenu() {
		...
		const btnAi = DOMEngine.create('button', 'menu-btn accent-green', menuScreen);
		...
		const backBtn = DOMEngine.create('button', 'menu-btn accent-ghost', menuScreen);
	...
	_showOnlineMenu() {
		...
		countDiv.style.cssText = 'display: flex; gap: 12px; margin-bottom: 24px; justify-content: center;';
		...
		[2, 3, 4].forEach(c => {
			...
			btn.onclick = () => {
				countDiv.querySelectorAll('button').forEach(b => b.classList.remove('active'));
				btn.classList.add('active');
				this.maxPlayers = c;
			};
		});
		
		const btnHost = DOMEngine.create('button', 'menu-btn accent-green', menuScreen);
		...
		const btnBack = DOMEngine.create('button', 'menu-btn accent-ghost', menuScreen);
	...
	_handleNetData(data, connId) {
		switch (data.type) {
			...
			break;
			case 'init':
			console.log('[Domino] guest received init, assignedId:', data.assignedId);
	...
	_startGame(mode, role = null) {
		...
		if (mode === 'online') {
			if (role === 'host') {
				this.players.forEach(p => { p.isLocal = false; });
	...
	_startRound(isHostFirstTime = false) {
		...
		if (this.startPlayer !== null) {
			...
			// Find 6-6
			for (let i = 0; i < this.players.length; i++) {
				const hasDoubleSix = this.players[i].hand.some(t => t[0] === 6 && t[1] === 6);
				if (hasDoubleSix) {
					this.turn = i;
					this.startPlayer = i;
					break;
	...
	_applyInitState(stateData) {
		...
		this._buildGameUI();
		DOMEngine.render();
		console.log('[Domino] guest UI built, state=', this.state, 'mode=', this.mode);
	},
	...
	_getValidMoves(hand) {
		if (this.boardTiles.length === 0) {
			if (this.startPlayer === this.turn && this.players[this.turn].hand.some(t => t[0]===6 && t[1]===6)) {
				return [{ tileIndex: this.players[this.turn].hand.findIndex(t => t[0]===6 && t[1]===6), sides: ['center'] }];
			}
		...
		hand.forEach((tile, i) => {
			const sides = [];
			if (tile[0] === this.boardEnds.left || tile[1] === this.boardEnds.left) sides.push('left');
			if (tile[0] === this.boardEnds.right || tile[1] === this.boardEnds.right) {
	...
	_endRound(winnerIndex, reason) {
		if (winnerIndex !== null) {
			...
			if (this.players[winnerIndex].score >= 100) {
				DOMEngine.showOverlay(`${pName} GANA LA PARTIDA!`, `Puntuación total: ${this.players[winnerIndex].score}`, () => {
					this.state = 'select';
					this._buildMenu();
				});
			} else {
				const msg = reason === 'domino' ? '¡Dominó!' : '¡Trancado!';
				DOMEngine.showOverlay(msg, `${pName} gana la ronda (+${totalPoints} pts).`, () => {
					this._startRound();
					DOMEngine.render();
				});
			}
			...
			// Tie
			DOMEngine.showOverlay('Empate', 'Juego trancado con empate. Nadie puntúa.', () => {
				this._startRound();
				DOMEngine.render();
			});
		}
	...
	_playAITurn() {
		...
		if (moves.length === 0) {
			...
			if (this.mode === 'online' && this.myRole === 'host') {
				this._broadcast({ type: 'play', playerIndex: this.turn, tileIndex: move.tileIndex, side });
			}
	...
	_buildGameUI() {
		...
		const posOrder = this.players.length === 2 ? ['top'] :
		this.players.length === 3 ? ['left', 'top'] : ['left', 'top', 'right'];
		...
		const drawBtnMobile = DOMEngine.create('button', 'game-action-btn', actions);
	...
	_renderGame() {
		...
		const posList = playerCount === 2 ? ['bottom', 'top'] :
		playerCount === 3 ? ['bottom', 'left', 'top'] : allPos;
		...
		bottomInfo.textContent = myPlayer.name + (myPlayer.id === this.turn ? ' — Tu turno' : '');
		if (myPlayer.id === this.turn) DOMEngine.addClass(bottomInfo, 'active-turn');
		else DOMEngine.removeClass(bottomInfo, 'active-turn');
		
		const isCurrentLocalTurn = this.turn === myPlayer.id && myPlayer.isLocal;
		const validMoves = isCurrentLocalTurn ? this._getValidMoves(myPlayer.hand) : [];
		...
		if (this.boardTiles.length === 0) {
			if (this._isMyTurn() && this.selectedTileIndex !== -1) {
				const selMove = validMoves.find(m => m.tileIndex === this.selectedTileIndex);
				if (selMove) {
					const ind = DOMEngine.create('div', 'play-indicator', track);
					ind.textContent = 'JUGAR';
					ind.style.cssText = 'position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); z-index: 20;';
					ind.onclick = () => this._doPlayerMove(this.turn, this.selectedTileIndex, 'center');
				}
			}
		...
		if (centerIdx !== -1) {
			...
			let t1 = cb.tile[0], t2 = cb.tile[1];
			if (cb.isReversed) { t1 = cb.tile[1]; t2 = cb.tile[0]; }
			const isDouble = t1 === t2;
			const rot = isDouble ? 0 : -90;
			placements.push({ b: cb, x: 0, y: 0, rot, t1, t2, isDouble, skin: 'cream' });
			...
			const placeChain = (tiles, startX, startY, startDir, isRightChain) => {
				...
				tiles.forEach((b) => {
					let temp1 = b.tile[0], temp2 = b.tile[1];
					if (b.isReversed) { temp1 = b.tile[1]; temp2 = b.tile[0]; }
					...
					if (newDir !== dir) {
						if (dir === 0 && newDir === 1) { cx -= G + W/2; cy += W/2 + G; }
						if (dir === 1 && newDir === 2) { cy -= G + W/2; cx -= W/2 + G; }
						if (dir === 2 && newDir === 3) { cx += G + W/2; cy -= W/2 + G; }
						if (dir === 3 && newDir === 0) { cy += G + W/2; cx += W/2 + G; }
						dir = newDir;
					}
					...
					let tx, ty;
					if (dir === 0)      { tx = cx + extent/2; ty = cy; cx += extent + G; }
					else if (dir === 1) { tx = cx; ty = cy + extent/2; cy += extent + G; }
					else if (dir === 2) { tx = cx - extent/2; ty = cy; cx -= extent + G; }
					else                { tx = cx; ty = cy - extent/2; cy -= extent + G; }
					
					placements.push({ b, x: tx, y: ty, rot: r, t1: bt1, t2: bt2, isDouble: dbl, skin: 'cream' });
				});
				return { cx, cy, dir };
			};
			...
			rightEnd = placeChain(this.boardTiles.slice(centerIdx + 1), rStart, 0, 0, true);
			leftEnd = placeChain([...this.boardTiles.slice(0, centerIdx)].reverse(), lStart, 0, 2, false);
		}
		
		let minX = -areaW / 2, maxX = areaW / 2;
		let minY = -areaH / 2, maxY = areaH / 2;
		...
		if (this._isMyTurn() && this.selectedTileIndex !== -1) {
			...
			if (selMove) {
				...
				if (selMove.sides.includes('right')) {
					...
					ind.onclick = () => this._doPlayerMove(this.turn, this.selectedTileIndex, 'right');
				}
				if (selMove.sides.includes('left')) {
					...
					ind.onclick = () => this._doPlayerMove(this.turn, this.selectedTileIndex, 'left');
				}
			}
		}
	...
	_renderActionButtons(validMoves) {
		...
		const moves = isMyTurn ? (validMoves || this._getValidMoves(this.players[this.turn].hand)) : [];
