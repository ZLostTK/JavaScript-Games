...
const _isMobileLayout = () =>
  _isTouchDevice() || window.matchMedia("(display-mode: standalone)").matches;
...
function _initMobileButtons() {
  ...
  fireBtn.addEventListener(
    "touchstart",
    (e) => {
      _mobileFirePressed = !_mobileFirePressed;
      fireBtn.classList.toggle("toggled-on");
      e.preventDefault();
    },
    { passive: false },
  );

  bombBtn.addEventListener(
    "touchstart",
    (e) => {
      _mobileBombPressed = true;
      e.preventDefault();
    },
    { passive: false },
  );
...
function _updateMobileSkillUI(powers) {
  ...
  fireBtn.innerHTML =
    ...
    '<span class="skill-label">FUEGO</span>';
  fireBtn.style.setProperty(
    "--skill-color",
    hasLaser ? "#ff0" : hasSpread ? "#0ff" : "#0ff",
  );
...
function renderParts(ctx) {
  for (const p of _parts) {
    ...
    if (p.trail) {
      ...
      ctx.beginPath();
      ctx.arc(p.x, p.y, Math.max(0.1, r), 0, TAU);
      ctx.fill();
    }
    ctx.restore();
  }
...
function renderStars(ctx) {
  for (const s of _stars) {
    ctx.globalAlpha =
      s.b * (0.5 + 0.5 * Math.sin(Date.now() * 0.001 + s.phase));
    ctx.fillStyle = "#fff";
    ctx.beginPath();
...
class Enemy {
  ...
  update(dt, target, bullets) {
    ...
    const lrp = (t) => {
      this.vx = lerp(this.vx, Math.cos(t) * this.spd, dt * 3);
      this.vy = lerp(this.vy, Math.sin(t) * this.spd, dt * 3);
    };
    if (this.state === "chase") lrp(ta);
    else if (this.state === "patrol") {
      this.vx = lerp(
        this.vx,
        Math.cos(this.patrolAngle) * this.spd * 0.5,
        dt * 2,
      );
      this.vy = lerp(
        this.vy,
        Math.sin(this.patrolAngle) * this.spd * 0.5,
        dt * 2,
      );
    } else if (this.state === "strafe") lrp(ta + Math.PI / 2);
    else if (this.state === "approach") {
      this.vx = lerp(this.vx, Math.cos(ta) * this.spd * 0.7, dt * 2);
      this.vy = lerp(this.vy, Math.sin(ta) * this.spd * 0.7, dt * 2);
    } else if (this.state === "kamikaze") {
      this.vx = lerp(this.vx, Math.cos(ta) * this.spd * 1.8, dt * 4);
      this.vy = lerp(this.vy, Math.sin(ta) * this.spd * 1.8, dt * 4);
    ...
    if (this.fireTimer <= 0 && d < 380) {
      const inac =
        this.type === "bomber" ? 0.28 : this.type === "scout" ? 0.14 : 0.05;
      bullets.push(
        new Bullet(
          this.x,
          this.y,
  ...
  render(ctx) {
    ...
    if (this.hp < this.maxHp) {
      const bw = this.r * 2;
      ctx.fillStyle = "#300";
      ctx.fillRect(this.x - bw / 2, this.y - this.r - 9, bw, 4);
      ctx.fillStyle = "#f40";
      ctx.fillRect(
        this.x - bw / 2,
        this.y - this.r - 9,
        bw * (this.hp / this.maxHp),
        4,
      );
    }
  }
...
class PowerUp {
  ...
  render(ctx) {
    ...
    ctx.translate(this.x, this.y);
    ctx.rotate(this.rot);
    ctx.strokeStyle = PU_COLORS[this.type];
    ctx.lineWidth = 1.5;
    ctx.shadowBlur = 14 * pulse;
    ctx.shadowColor = PU_COLORS[this.type];
    ctx.beginPath();
...
class Player {
  ...
  update(dt, bullets, overrides = {}) {
    ...
    const rotLeft =
      overrides.rotLeft ?? (Input.isDown("ArrowLeft") || Input.isDown("KeyA"));
    const rotRight =
      overrides.rotRight ??
    ...
    if (_joyActive) {
      ...
      if (touch) {
        ...
        if (tg.x < W * 0.6) {
          ...
          while (da < -Math.PI) da += TAU;
          this.rot +=
            Math.sign(da) * Math.min(Math.abs(da), this.rotSpd * dt * 3);
    ...
    const thrusting =
      overrides.thrust ??
      (Input.isDown("ArrowUp") || Input.isDown("KeyW") || thrustTouch);
    if (thrusting) {
      this.vx += Math.cos(this.rot) * this.thrust * spdMul * dt;
      this.vy += Math.sin(this.rot) * this.thrust * spdMul * dt;
      if (Math.random() < 0.45)
        spawnParts(
          this.x - Math.cos(this.rot) * 15,
          this.y - Math.sin(this.rot) * 15,
          1,
          {
            color: Math.random() < 0.5 ? "#f80" : "#ff0",
            minSpd: 30,
            maxSpd: 70,
            angle: this.rot + Math.PI,
            spread: 0.45,
            minDecay: 4,
            maxDecay: 7,
            minR: 1.5,
    ...
    if (shooting && this.fireTimer <= 0 && this.energy > 0) {
      ...
      const bx = this.x + Math.cos(this.rot) * 16,
        by = this.y + Math.sin(this.rot) * 16;
      if (this.powers.laser > 0) {
        bullets.push(
          new Bullet(bx, by, this.rot, 860, "player", 3, "#ff0", true),
        );
        if (game.onlineMode === "1v1")
          Online.send({
            type: "bulletSpawn",
            x: bx,
            y: by,
        ...
        for (const da of [-0.27, 0, 0.27]) {
          bullets.push(
            new Bullet(bx, by, this.rot + da, 500, "player", 1, this.color),
          );
          if (game.onlineMode === "1v1")
            Online.send({
              type: "bulletSpawn",
        ...
        bullets.push(
          new Bullet(bx, by, this.rot, 500, "player", 1, this.color),
        );
        if (game.onlineMode === "1v1")
          Online.send({
            type: "bulletSpawn",
    ...
    this._nova = false;
    const bombPressed =
      overrides.bomb ?? (Input.isPressed("KeyX") || _mobileBombPressed);
    if (bombPressed && this.powers.bomb > 0 && this.energy >= 30) {
      this.powers.bomb = 0;
      this.energy -= 30;
      this._nova = true;
      spawnParts(this.x, this.y, 70, {
        color: "#f80",
        minSpd: 80,
        maxSpd: 380,
        minR: 2,
  ...
  render(ctx) {
    ...
    ctx.closePath();
    ctx.stroke();
    ctx.strokeStyle = this.isRemote ? this.color + "88" : "#08f";
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(-5, -4);
    ctx.lineTo(-9, -4);
    ...
    if (this.powers.shield > 0) {
      ...
      ctx.shadowColor = "#0f0";
      ctx.beginPath();
      ctx.arc(this.x, this.y, 24 * p2, 0, TAU);
      ctx.stroke();
      ctx.restore();
    }
...
const game = {
  ...
  powerups: [],
  score: 0,
  hiScore: +localStorage.getItem("void-sector-hi") || 0,
  wave: 1,
  combo: 1,
  comboTimer: 0,
  waveMsg: "",
  ...
  _startGame(mode, role) {
    ...
    if (mode === "coop") {
      ...
      this.player = new Player(
        isHost ? W * 0.35 : W * 0.65,
        H / 2,
        isHost ? "#0ff" : "#f80",
        false,
      );
      this.player2 = new Player(
        isHost ? W * 0.65 : W * 0.35,
        H / 2,
        isHost ? "#f80" : "#0ff",
        true,
      );
  ...
  _spawnWave(w) {
    ...
    for (let i = 0; i < 3 + w * 2; i++) {
      ...
      do {
        x = rnd(0, W);
        y = rnd(0, H);
      } while (dist({ x, y }, this.player) < 120);
    ...
    if (w >= 2)
      for (let i = 0; i < Math.min(w - 1, 4); i++) {
        ...
        do {
          x = rnd(0, W);
          y = rnd(0, H);
        } while (dist({ x, y }, this.player) < 180);
    ...
    if (w >= 3 && w % 2 === 0) {
      ...
      do {
        x = rnd(0, W);
        y = rnd(0, H);
      } while (dist({ x, y }, this.player) < 220);
    ...
    if (w >= 5 && w % 3 === 0) {
      ...
      do {
        x = rnd(0, W);
        y = rnd(0, H);
      } while (dist({ x, y }, this.player) < 260);
      this.enemies.push(new Enemy(x, y, "bomber"));
    }
    this.waveMsg = `OLEADA ${w}`;
    this.waveMsgTimer = 2.2;
  ...
  update(dt) {
    ...
    for (const k in this._btns)
      this._hover[k] = UICanvas.hitTest(gp.x, gp.y, this._btns[k]);

    if (this.state === "menu") {
      _showMobileControls(false);
      if (this._isPointerPressed()) {
        if (UICanvas.hitTest(gp.x, gp.y, this._btns.play))
          this._startGame(null, null);
    ...
    if (this.state === "dead") {
      ...
      if (this.deathTimer <= 0 && this._isPointerPressed()) {
        if (this.player.lives > 0) {
          this._respawn();
          if (this.onlineMode) Online.send({ type: "restart" });
        } else this.state = "gameover";
      }
      if (
        this.deathTimer <= 0 &&
        (Input.isPressed("Space") || Input.isPressed("Enter"))
      ) {
      ...
      if (b.owner === "player") {
        for (let ai = 0; ai < this.asteroids.length; ai++) {
          ...
          if (!b.dead && dist(b, a) < a.r + b.r) {
            ...
            // Tell peer
            if (this.onlineMode)
              Online.send({ type: "asteroidHit", idx: ai, dmg: b.dmg });
          }
        }
        for (let ei = 0; ei < this.enemies.length; ei++) {
          ...
          if (!b.dead && dist(b, e) < e.r + b.r) {
            ...
            if (this.onlineMode)
              Online.send({ type: "enemyHit", idx: ei, dmg: b.dmg });
          }
        }
        ...
        if (
          ...
          this.player2 &&
          !b.dead &&
          dist(b, this.player2) < this.player2.r + b.r
        ) {
          b.dead = true;
    ...
    for (let i = this.asteroids.length - 1; i >= 0; i--) {
      ...
      if (a.hp <= 0) {
        ...
        spawnParts(a.x, a.y, 10 + a.size * 5, {
          color: "#889",
          minSpd: 30,
          maxSpd: 110,
        });
    ...
    const getTarget = () => {
      if (this.onlineMode === "coop" && this.player2) {
        return dist(this.enemies[0] || p, p) <
          dist(this.enemies[0] || p, this.player2)
          ? p
          : this.player2;
      }
      return p;
  ...
  render(ctx) {
    ...
    if (this.state === "dead") {
      ...
      ctx.shadowColor = "#f44";
      ctx.fillStyle = "#f84";
      ctx.fillText("NAVE DESTRUIDA", W / 2, H / 2 - 24);
      ctx.restore();
  ...
  _renderHUD(ctx) {
    ...
    ctx.fillStyle = "#0ff";
    ctx.shadowBlur = 6;
    ctx.fillText(String(this.wave).padStart(2, "0"), W - 12, 24);
    ...
    for (let i = 0; i < puKeys.length; i++) {
      const k = puKeys[i];
      const px2 = W / 2 - puKeys.length * 20 + i * 40;
      ctx.shadowBlur = 10;
      ctx.shadowColor = PU_COLORS[k];
      ctx.strokeStyle = PU_COLORS[k];
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.roundRect(px2 - 18, H - 44, 36, 32, 5);
      ctx.stroke();
  ...
  _renderMenu(ctx) {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.shadowBlur = 28 + 8 * Math.sin(t * 2);
    ctx.shadowColor = "#0ff";
    ctx.fillStyle = "#0ff";
    ctx.fillText("VOID", W / 2, H / 2 - 200);
    ctx.fillStyle = "#08f";
    ctx.fillText("SECTOR", W / 2, H / 2 - 148);
    ctx.shadowBlur = 0;
    ...
    if (this.hiScore > 0) {
      ...
      ctx.fillStyle = "#aef";
      ctx.fillText(
        `REC  ${String(this.hiScore).padStart(6, "0")}`,
        W / 2,
        H / 2 + 130,
      );
    }
  ...
  _openOnline() {
    ...
    hostB.onclick = () => {
      ...
      hostB.disabled = true;
      hostB.innerHTML =
        '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 4px;"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg> Creando...';
      Online.destroy();
      Online.host((code) => {
        OnlineLobby.setCode(code);
        OnlineLobby.setStatus(
          `Esperando jugador... (${selMode.toUpperCase()})`,
        );
