...
export class Input {
	static init(canvas) {
		...
		this._onKeyDown = (e) => {
			...
			const payload = { code: e.code, key: e.key };
			EventBus.emit(Events.INPUT_KEY_DOWN, payload);
			EventBus.emit(Events.INPUT_KEY_PRESSED, payload);
			e.preventDefault();
		};
		...
		this._onKeyUp = (e) => {
			...
			EventBus.emit(Events.INPUT_KEY_UP, { code: e.code, key: e.key });
			e.preventDefault();
		};
