...
class UIMenu {
  ...
  draw(ctx, overrides = {}) {
    const drawCtx =
      ctx || (typeof RenderBridge !== "undefined" ? RenderBridge.ctx : null);
    if (!drawCtx) return;
  ...
  handleInput(opts = {}) {
    ...
    if (
      skipIfLobbyVisible &&
      typeof OnlineLobby !== "undefined" &&
      OnlineLobby.isVisible()
    ) {
      return null;
    }
  ...
  static onlineSetup(opts = {}) {
    return new UIMenu(
      [
        {
          id: "host",
          label: opts.hostLabel ?? "Crear partida",
          y: 140,
          accent: Theme.colors.success,
        },
        {
          id: "join",
          label: opts.joinLabel ?? "Unirse",
          y: 210,
          accent: Theme.colors.accent2,
        },
        {
          id: "back",
          label: opts.backLabel ?? "← Volver",
          y: 310,
