...
class GameBoot {
	...
	static _resolveRenderer(opts = {}) {
		...
		if (typeof PIXIEngine !== 'undefined' && opts.engine === PIXIEngine) return 'pixi';
		if (typeof LittleEngine !== 'undefined' && opts.engine === LittleEngine) return 'little';
		if (typeof Engine !== 'undefined' && opts.engine === Engine) return 'canvas';
		return 'canvas';
	}
	...
	static startPIXI(game, opts = {}) {
		if (typeof PIXIEngine === 'undefined') throw new Error('GameBoot.startPIXI: carga pixi.min.js y pixi-engine.js');
		this._whenReady(async () => {
			await PIXIEngine.init(opts.containerId || opts.canvasId || 'game-container', {
				width: opts.width || 800,
				height: opts.height || 600,
				bg: opts.bg ?? 0x0f0f1a,
			});
	...
	static startLittle(game, opts = {}) {
		if (typeof LittleEngine === 'undefined') throw new Error('GameBoot.startLittle: carga littlejs.min.js y littlejs-engine.js');
		this._whenReady(() => {
	...
	static startDOM(game, opts = {}) {
		this._whenReady(() => {
			DOMEngine.init(opts.containerId || 'game-container', { fps: opts.fps || 60 });
