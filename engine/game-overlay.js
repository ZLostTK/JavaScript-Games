/** Overlay de fin de partida unificado — dim + título + puntuación + hint */
class GameOverlay {
	static draw(ctx, opts = {}) {
		...
		if (score !== undefined && score !== null) {
			const scoreText = typeof score === 'number' || typeof score === 'string'
				? `${scoreLabel}: ${score}`
				: String(score);
	...
	/** Mensajes habituales para multijugador online */
	static onlineResult(winner, myId, opts = {}) {
		...
		if (winner === 'draw' || winner === 0) {
			return { title: opts.drawText ?? '¡Empate!', color: Theme.colors.warning };
		}
		const won = winner === myId;
		return {
			title: won ? (opts.winText ?? '¡Ganaste! 🎉') : (opts.loseText ?? '¡Perdiste!'),
